...
#fungsi memfilter data supaya tidak terlalu banyak yang dibebankan di model
def prosesData(user_input):
    #mencari keyword masuk di kolom supplier atau barang
    filtered_df1 = df[df['Barang'].str.contains(user_input, case=False,na=False)]
    filtered_df2 = df[df['Supplier'].str.contains(user_input, case=False,na=False)]

    #memasukan data yang sudah dicari (kenapa merge? supaya mudah jika keyword di barang/supplier)
    merge_df = pd.merge(filtered_df1, filtered_df2, on='PO Number')
...
def chat():
    try:
        ...
        #mengambil df yang sudah difilter
        df = prosesData(user_input)
